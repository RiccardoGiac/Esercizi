...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=10) as executor: #threadpoolexecutor mi permette di creare tanti thread quanti sono i maxworkers
        executor.map(funzione, range(100))#map prende un elemento alla volta di range(100) e lo usa come parametro nella funzione
    ...
    #     x: threading.Thread = threading.Thread(target=funzione, args=(id,)) id con virgola così viene una tupla
